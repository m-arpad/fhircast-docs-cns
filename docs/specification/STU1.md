...
A subscriber may request to add, remove, or change one or more FHIR resources to a context container with an HTTP POST to the
`hub.topic` url.  The Hub SHALL either acknowledge that the request was honored successfully by returning a success HTTP status
(200 - OK) or returning one of the below HTTP status codes if the request was not successful.  The Hub SHALL perform all requested
operataions atomically and all requested operations must either succeed or the Hub MUST fail the entire request and return one of
the below errors. The subscriber SHALL be able to appropriately handle a failed update request.

###### Request Update Content Failure - HTTP Status Codes
...
403	| Forbidden	| Update or removal of one of the resources requested to be updated or removed is not permitted by any application except the creator of the resource
...
428 | Precondition Required | The timestamp in the submitted request is not the current version of the content held by the Hub
...
Each resource MAY add the `client.id` to the source attribute of the resource’s meta attribute.

In the tag attribute of a resource’s meta attribute an application MAY specify one or more protection levels for the resource (in the
above example, the Observation resource contains an update restriction while the ImagingStudy resource does not specify any protection
level).  The following protection levels are available in the http://fhircast.org/exchange/protection code set:
...
403 | lock-error | MSG_RESOURCE_NOT_ALLOWED | Removal or update of one of the requested resource is not permitted by any application except the creator of the resource
404 | not-found | MSG_NO_EXIST | One of the FHIR resources which is requested to be updated or removed was not found
...
##### Update Content Notification Parameters
The Hub SHALL send the 'update-content' event (see in Event Catalog) to all applications (including the submitting client) participating in the case if the request was
successfully completed. The Hub SHALL NOT send any notification to other applications if the request fails.
...
`container.id` | Required | string | Resource id of the contextual subject in which this content resides
`content.timestamp` | Required | string | Timestamp provided by the Hub when it last updated content
`content` | Required | Arrary | Resources which are present in the containing contextual subject
